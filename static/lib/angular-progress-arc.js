            ...
            template:
                '<svg ng-attr-width="{{size}}" ng-attr-height="{{size}}">' +
                    ...
                    '<circle class="ngpa-progress" fill="none"' +
												'ng-attr-stroke-linecap = "{{round ? \' round\' : \' none \'}}"' +
                        'ng-attr-cx="{{size/2}}" ' +
                        'ng-attr-cy="{{size/2}}" ' +
                        'ng-attr-r="{{radius}}" ' +
                        ...
                        '/>' +
										'<text style="text-anchor: middle;" class="ngpa-percent" '+
											'ng-attr-fill="{{textFill}}" ' +
											'ng-attr-x="{{size/2}}" ' +
											'ng-attr-y="{{size/2+(textPosition/2)}}">' +
											'{{percentage ? text() : (complete()*100) + \'%\'}}' +
										'<tspan ng-attr-x="{{size/2}}" ng-attr-y="{{size/2+(3*textPosition/2)}}">'+
										'{{suffix}}' +
										'</tspan>'+
										'</text>'+
                '</svg>'
